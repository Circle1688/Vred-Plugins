...
class XPengVRTools(form, base):
    ...
    def __init__(self, parent=None):
        ...
        super(XPengVRTools, self).__init__(parent)
        ...
        self._random_material = None
        self.uv_tool_dialog = None
        self.fetch_material_dialog = None
        self.file_dialog_path = os.path.join(os.path.expanduser("~"), 'Desktop')
        self.material_data_path = ""
    ...
    def setupUserInterface(self):
        ...
        self._label.setPixmap(QtGui.QPixmap(".\icon\icon_xp_vr.png").scaled(100, 50, QtCore.Qt.KeepAspectRatio))
        ...
        self._uvTools.setIcon(QtGui.QIcon(".\icon\icon_material_record.png"))
        self._uvTools.setIconSize(QtCore.QSize(32, 32))

        self._fetchMaterial.clicked.connect(self.fetchMaterial)
        self._fetchMaterial.setIcon(QtGui.QIcon(".\icon\icon_material_record.png"))
        self._fetchMaterial.setIconSize(QtCore.QSize(32, 32))
    ...
    def export2UE(self):
        ...
        if not self.dialog:
            ...
            create_random_Btn = QtWidgets.QPushButton('生成材质')
            create_random_Btn.setIcon(self.get_icon('icon_material.png'))
            create_random_Btn.setIconSize(QtCore.QSize(32, 32))
            create_random_Btn.clicked.connect(self.create_random_material)
            ...
            VBoxLayout.addWidget(create_random_Btn)
    ...
    def create_random_material(self):
        def reject():
            self._random_material = None
        
        def create_materials():
            text = list_edit.toPlainText()

            names = text.split('\n')
            self.create_random_materials(names)

            self._random_material.reject()
            

        if not self._random_material:
            self.dialog.reject()
            _label = QtWidgets.QLabel('命名列表:')
            list_edit = QtWidgets.QPlainTextEdit()
            create_Btn = QtWidgets.QPushButton('生成')

            create_Btn.clicked.connect(create_materials)

            VBoxLayout = QtWidgets.QVBoxLayout()
            VBoxLayout.addWidget(_label)
            VBoxLayout.addWidget(list_edit)
            VBoxLayout.addWidget(create_Btn)

            self._random_material = QtWidgets.QDialog(self)
            self._random_material.setLayout(VBoxLayout)
            self._random_material.setWindowTitle("使用UE材质重命名")
            self._random_material.setWindowIcon(self.get_icon('icon_material_panel.png'))
            self._random_material.resize(400, 600)

            screen = QtWidgets.QDesktopWidget().screenGeometry()
            size = self._random_material.geometry()
            self._random_material.move((screen.width() - size.width()) / 2, (screen.height() - size.height()) / 2)

            self._random_material.show()
            self._random_material.rejected.connect(reject)
    ...
    def _MessageBox(self, message):
        ...
        msgBox.setWindowTitle("XPengVRTools")
    ...
    def renameNode(self):
        ...
        self.renameNode_recursive(vrdNode(node))
    ...
    def datamaterials(self):
        ...
        def accept():
            if Alineedit.text() == '' or Blineedit.text() == '':
                ...
                # self.offset = float(Elineedit.text())
                self.match_option = combobox.currentIndex()
        ...
        if not self.matchdialog:
            ...
            Alabel = QtWidgets.QLabel("匹配类型:")

            combobox = QtWidgets.QComboBox()
            combobox.addItem('材质和UV')
            combobox.addItem('仅材质')
            combobox.addItem('仅UV')
            ...
            ABlabel = QtWidgets.QLabel("参考节点（老）:")
            ...
            BBlabel = QtWidgets.QLabel("匹配节点（新）:")

            AAAlabel = QtWidgets.QLabel()

            Infolabel = QtWidgets.QLabel("可以移动参考节点（老），切勿移动匹配节点（新）！")
    ...
    def materialsCore(self, oldnode, newnode):
        ...
        def copy_uv(old, new):
            project_mode = vrUVService.getProjectionMode(vrdGeometryNode(vrdNode(old)), uvSet=vrUVTypes.MaterialUVSet)
            if project_mode == vrUVTypes.UVProjectionMode.PlanarMapping:
                ProjectionSettings = vrUVService.readPlanarProjectionSettings(vrdGeometryNode(vrdNode(old)), uvSet=vrUVTypes.MaterialUVSet)
                vrUVService.applyPlanarProjection([vrdGeometryNode(vrdNode(new))], ProjectionSettings, uvSet=vrUVTypes.MaterialUVSet)

            elif project_mode == vrUVTypes.UVProjectionMode.TriplanarMapping:
                ProjectionSettings = vrUVService.readTriplanarProjectionSettings(vrdGeometryNode(vrdNode(old)), uvSet=vrUVTypes.MaterialUVSet)
                vrUVService.applyTriplanarProjection([vrdGeometryNode(vrdNode(new))], ProjectionSettings, uvSet=vrUVTypes.MaterialUVSet)
                
            elif project_mode == vrUVTypes.UVProjectionMode.CylindricalMapping:
                ProjectionSettings = vrUVService.readCylindricalProjectionSettings(vrdGeometryNode(vrdNode(old)), uvSet=vrUVTypes.MaterialUVSet)
                vrUVService.applyCylindricalProjection([vrdGeometryNode(vrdNode(new))], ProjectionSettings, uvSet=vrUVTypes.MaterialUVSet)
        ...
        for newgeonode in newgeonodes:
            ...
            # newvector = vrOSGTypes.Vec3f(newvector.x(), newvector.y() - self.offset, newvector.z())
            

            scores = {}
            for oldgeonode in oldgeonodes:

                oldvector = vrNodeUtils.getBoundingBoxCenter(oldgeonode, False)  # 获取边界框中心位置

                oldbxvector = getBoundingBoxVector(oldgeonode)  # 获取边界框对角线向量
                oldbxsize = getvectorlength(oldbxvector)
                center_dist = CalVectorLength(oldvector, newvector)  # 计算中心点差值
                size_sim = cos_sim(oldbxvector, newbxvector)  # 计算边界框对角线向量相似度
                # bxsizediff = Calpercent(oldbxsize, newbxsize) * 100  # 计算边界框向量长度相似度

                # 边界框得分
                bbox_score = (1 - center_dist / 20) * 0.5 + size_sim * 0.5

                # 计算三角形相似度
                a = vrdGeometryNode(oldgeonode).getPrimitiveCount()
                b = vrdGeometryNode(newgeonode).getPrimitiveCount()
                tri_count_radio = Calpercent(a, b)

                v_a = vrdGeometryNode(oldgeonode).getVertexCount()
                v_b = vrdGeometryNode(newgeonode).getVertexCount()
                vertex_count_radio = Calpercent(v_a, v_b)

                # 网格体得分
                geo_score = tri_count_radio * 0.6 + vertex_count_radio * 0.3

                # _thresholdValue = self.thresholdValue  # 获得中心点阈值
                # sizeThresholdValue = self.sizethresholdValue  # 定义边界框大小阈值

                # 计算总分
                score = bbox_score * 0.8 + geo_score * 0.2

                scores[oldgeonode] = score
            
            max_node = max(scores, key=scores.get)  # 取得分最高的节点
            ...
            if self.match_option == 0:
                newgeonode.applyMaterial(max_node.getMaterial())  # 获取节点并替换材质和uv
                copy_uv(max_node, newgeonode)
            elif self.match_option == 1:
                newgeonode.applyMaterial(max_node.getMaterial())  # 获取节点并替换材质
            elif self.match_option == 2:
                copy_uv(max_node, newgeonode)  #仅替换uv
    ...
    def uv_tools(self):
        ...
        def click_record():
            node = vrScenegraph.getSelectedNode()
            mat_lineedit.setText(vrdNode(node).getName())
        ...
        if not self.uv_tool_dialog:
            ...
            self.uv_tool_dialog = QtWidgets.QDialog(self)
            self.uv_tool_dialog.setLayout(vbox)
            self.uv_tool_dialog.setWindowTitle('UV工具')
            self.uv_tool_dialog.setWindowIcon(self.get_icon('icon_material_panel.png'))
            self.uv_tool_dialog.resize(400, 100)

            screen = QtWidgets.QDesktopWidget().screenGeometry()
            size = self.uv_tool_dialog.geometry()
            self.uv_tool_dialog.move((screen.width() - size.width()) / 2, (screen.height() - size.height()) / 2)

            self.uv_tool_dialog.show()
            self.uv_tool_dialog.rejected.connect(reject)

    def fetchMaterial(self):

        def opendialog():
            filedialog = QtWidgets.QFileDialog()
            fileopenpath = filedialog.getOpenFileName(self.fetch_material_dialog, '导入Omniverse材质文件', self.file_dialog_path,'Omniverse材质文件(*.ovmt)')[0]

            if fileopenpath != "":
                self.file_dialog_path = os.path.dirname(fileopenpath)
                self.material_data_path = fileopenpath
                Alineedit.setText(fileopenpath)

        def reject():
            self.fetch_material_dialog = None

        def fetch():
            if self.material_data_path != "":
                with open(self.material_data_path, "r") as f:
                    content = f.read()
                    ...
                    self._MessageBox("同步完成！")




        if not self.fetch_material_dialog:
            Alabel = QtWidgets.QLabel('ovmt文件：')
            Alineedit = QtWidgets.QLineEdit()
            Alineedit.setReadOnly(True)
            Alineedit.setText(self.material_data_path)
            Bopen = QtWidgets.QPushButton('同步')
            Bopen.setIcon(QtGui.QIcon(self.get_icon('icon_csv.png')))
            Bopen.setIconSize(QtCore.QSize(32, 32))
            Bopen.clicked.connect(fetch)

            open_btn = QtWidgets.QPushButton('...')
            open_btn.setFixedWidth(45)
            open_btn.clicked.connect(opendialog)
            ...
            vbox.addLayout(hbox)
            vbox.addWidget(Bopen)

            self.fetch_material_dialog = QtWidgets.QDialog(self)
            self.fetch_material_dialog.setLayout(vbox)
            self.fetch_material_dialog.setWindowTitle('同步材质')
            self.fetch_material_dialog.setWindowIcon(self.get_icon('icon_material_panel.png'))
            self.fetch_material_dialog.resize(400, 100)

            screen = QtWidgets.QDesktopWidget().screenGeometry()
            size = self.fetch_material_dialog.geometry()
            self.fetch_material_dialog.move((screen.width() - size.width()) / 2, (screen.height() - size.height()) / 2)

            self.fetch_material_dialog.show()
            self.fetch_material_dialog.rejected.connect(reject)


# Actually start the plugin
if not importError:
    XPengVRTools = XPengVRTools(VREDPluginWidget)